...
for task in task_map:
    ...
    for epoch in tqdm(range(config.downstream_epochs), leave=False, desc=f'{task} Tuning'):
        ...
        for batch_images, _, batch_labels in tqdm(task_tune_loader, desc=f'{task} Tuning Epoch {epoch+1}', leave=False):
    ...
    task_preds = []
    task_labels = []
    for batch_images, _, batch_labels in tqdm(task_test_loader, desc=f'{task} Testing', leave=False):
        batch_images = batch_images.to(device)
        batch_labels = batch_labels.to(device)
        with torch.no_grad():
            representations = torch.randn(batch_images.size(0), config.representation_size).to(device)
