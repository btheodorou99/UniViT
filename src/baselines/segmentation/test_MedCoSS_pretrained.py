...
for task in valid_tasks:
    ...
    task_tune_data = ImageDataset(task_tune, config, "cpu", patch_size=16, image_size=config.max_height, image_depth=config.max_depth)
    ...
    task_test_data = ImageDataset(task_test, config, "cpu", patch_size=16, image_size=config.max_height, image_depth=config.max_depth)
    ...
    downstream = SegmentationModel(EMBEDDING_DIM, 16, config.max_depth).to(device)
    ...
    for epoch in tqdm(
        ...
        for batch_images, batch_labels in tqdm(
            ...
            with torch.no_grad():
                representations = model.embed_patches({
                    "data": batch_images,
                    "modality": "3D image"
                })
