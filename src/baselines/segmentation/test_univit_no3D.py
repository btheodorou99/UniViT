...
for task in valid_tasks:
    ...
    for epoch in tqdm(
        ...
        for batch_images, batch_dimensions, batch_labels in tqdm(
            ...
            batch_dimensions = batch_dimensions[:, [0, 2, 3]]
            batch_images = batch_images.permute(0,2,1,3,4,5)
            batch_images = batch_images.view(-1, config.max_time, config.num_channels, config.max_height, config.max_width)
    ...
    for batch_images, batch_dimensions, batch_labels in tqdm(
        ...
        # Flatten everything for 2D segmentation
        bs, time, depth, channels, height, width = batch_images.shape
        batch_dimensions = batch_dimensions[:, [0, 2, 3]]
        batch_images = batch_images.permute(0,2,1,3,4,5)
        batch_images = batch_images.view(-1, time, channels, height, width)
        with torch.no_grad():
            representations = model.embed_patches(batch_images, batch_dimensions)
            predictions = downstream(representations)
