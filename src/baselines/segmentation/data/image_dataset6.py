...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = F.pad(img, (0, 0, 0, 0, 0, 0, self.config.depth_patch_size + self.config.depth_patch_size // 2, self.config.max_depth - self.config.depth_patch_size - self.config.depth_patch_size // 2 - 1))
        ...
        dimension_tensor[-(self.config.max_depth - self.config.depth_patch_size - self.config.depth_patch_size // 2 - 1):, 1] = torch.tensor([i for i in reversed(range(self.config.depth_patch_size + self.config.depth_patch_size // 2 + 1, self.config.max_depth))])
