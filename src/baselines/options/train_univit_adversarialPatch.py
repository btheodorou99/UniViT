...
save_diar = '/srv/local/data/bpt3/UniViT/save'
...
if os.path.exists(f"{save_dir}/univit_adversarialPatch.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'{save_dir}/univit_adversarialPatch.pt', map_location='cpu')
...
while num_steps < config.tot_steps:
    ...
    for batch_images, batch_dimensions in train_loader:
        ...
        cls_model1, embd_seq_model1, mask1, orig_mask1 = model(batch_images1.to(device), batch_dimensions1.to(device), seqMask=True, train=True, adversarialPatch=adversarial)
        cls_model2, embd_seq_model2, mask2, orig_mask2 = model(batch_images2.to(device), batch_dimensions2.to(device), seqMask=True, train=True, adversarialPatch=adversarial)
        with torch.no_grad():
        ...
        adv_cls_model1 = model.embed(batch_images1.to(device), batch_dimensions1.to(device), train=True, adversarialPatch=adversarial)
        adv_cls_model2 = model.embed(batch_images2.to(device), batch_dimensions2.to(device), train=True, adversarialPatch=adversarial)
        adv_loss_cls = - (cls_loss_fn(adv_cls_model1, cls_model1, center_cls) + cls_loss_fn(cls_model2, cls_model2, center_cls)) / 2
        ...
        if num_steps % 1000 == 0:
            ...
            torch.save({'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'steps': num_steps}, f'{save_dir}/univit_adversarialPatch.pt')
