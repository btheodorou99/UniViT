...
if os.path.exists(f"{save_dir}/univit_nextFrameSimilarity.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'{save_dir}/univit_nextFrameSimilarity.pt', map_location='cpu')
...
while num_steps < config.tot_steps:
    ...
    for batch_images, batch_dimensions in train_loader:
        ...
        cls_model1, embd_seq_model1, mask1, orig_mask1, time_mask1, slice_mask1 = model(batch_images1.to(device), batch_dimensions1.to(device), seqMask=True, train=True)
        cls_model2, embd_seq_model2, mask2, orig_mask2, time_mask2, slice_mask2 = model(batch_images2.to(device), batch_dimensions2.to(device), seqMask=True, train=True)
        ...
        if num_steps % 1000 == 0:
            ...
            torch.save({'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'steps': num_steps}, f'{save_dir}/univit_nextFrameSimilarity.pt')
