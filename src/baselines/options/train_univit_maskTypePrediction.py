...
if os.path.exists(f"{save_dir}/univit_maskTypePrediction.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'{save_dir}/univit_maskTypePrediction.pt', map_location='cpu')
...
def maskType_loss_fn(student_emb, mask, orig_mask):
    student_emb = student_emb.view(-1)
    orig_mask = orig_mask.view(-1)
    mask = mask.view(-1)
    student_emb = student_emb[mask == 1]
    orig_mask = orig_mask[mask == 1]
    loss = F.binary_cross_entropy_with_logits(student_emb, orig_mask)
    return loss
...
while num_steps < config.tot_steps:
    ...
    for batch_images, batch_dimensions in train_loader:
        ...
        cls_model1, embd_seq_model1, pos_out_model1, mask1, orig_mask1 = model(batch_images1.to(device), batch_dimensions1.to(device), train=True, seqMask=True, posOutput=True)
        cls_model2, embd_seq_model2, pos_out_model2, mask2, orig_mask2 = model(batch_images2.to(device), batch_dimensions2.to(device), train=True, seqMask=True, posOutput=True)
        with torch.no_grad():
        ...
        loss_maskType = (maskType_loss_fn(pos_out_model1, mask1, orig_mask1) + maskType_loss_fn(pos_out_model2, mask2, orig_mask2)) / 2
        ...
        if num_steps % 1000 == 0:
            ...
            torch.save({'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'steps': num_steps}, f'{save_dir}/univit_maskTypePrediction.pt')
        if num_steps >= config.tot_steps:
            break
  
pbar.close()
pickle.dump(loss_plot, open(f'{save_dir}/univit_maskTypePrediction_loss_plot.pkl', 'wb'))