...
if os.path.exists(f"{save_dir}/univit_maskPosEmbedding.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'{save_dir}/univit_maskPosEmbedding.pt', map_location='cpu')
...
def maskPos_loss_fn(student_emb, orig_mask, pos_mask):
    bs, seq_len, _ = student_emb.size()
    pos_mask = pos_mask & (~orig_mask)
    
    true_indices = pos_mask.nonzero(as_tuple=True)
    targets_time = true_indices[1]
    targets_depth = true_indices[2]
    targets_height = true_indices[3]
    targets_width = true_indices[4]
    
    logits_time = student_emb[:, :config.max_time]
    logits_depth = student_emb[:, config.max_time:config.max_time+config.max_slice]
    logits_height = student_emb[:, config.max_time+config.max_slice:config.max_time+config.max_slice+config.max_height]
    logits_width = student_emb[:, config.max_time+config.max_slice+config.max_height:]
    
    pos_mask = pos_mask.view(bs, seq_len)
    
    loss_time = F.cross_entropy(logits_time[pos_mask].view(-1, config.max_time), targets_time)
    loss_depth = F.cross_entropy(logits_depth[pos_mask].view(-1, config.max_depth), targets_depth)
    loss_height = F.cross_entropy(logits_height[pos_mask].view(-1, config.max_height), targets_height)
    loss_width = F.cross_entropy(logits_width[pos_mask].view(-1, config.max_width), targets_width)
    
    loss = loss_time + loss_depth + loss_height + loss_width
    return loss
...
while num_steps < config.tot_steps:
    ...
    for batch_images, batch_dimensions in train_loader:
        ...
        embd_seq_model, pos_embd_seq_model, mask, orig_mask, pos_mask = model(batch_images, batch_dimensions, seqMask=True, posMask=True, train=True, posOutput=True)
        with torch.no_grad():
        ...
        loss_maskType = maskPos_loss_fn(pos_embd_seq_model, orig_mask, pos_mask)
        loss = loss_cls + loss_mim + loss_maskType
        ...
        if num_steps % 1000 == 0 and batches_since_step % batches_per_step == 0:
            ...
            torch.save({'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'steps': num_steps}, f'{save_dir}/univit_maskPosEmbedding.pt')
        if num_steps >= config.tot_steps:
            break
  
pbar.close()
pickle.dump(loss_plot, open(f'{save_dir}/univit_maskPosEmbedding_loss_plot.pkl', 'wb'))