...
while num_steps < config.tot_steps:
    ...
    for batch_images, batch_dimensions in train_loader:
        ...
        cls_model = model.embed(batch_images_aug1, batch_dimensions1, train=True, patchMask=True)
        embd_seq_model, mask, orig_mask = model(batch_images, batch_dimensions, seqMask=True, train=True, patchMask=True)
        with torch.no_grad():
