...
modalities = list(set([p[0][3] for p in train_data_all]))
knn_data = {
...
for modality in modalities:
    ...
    if os.path.exists(f"{save_dir}/univit_individual_{save_name}.pt"):
        ...
        checkpoint = torch.load(
            f"{save_dir}/univit_individual_{save_name}.pt", map_location="cpu"
        )
    ...
    model.train()
    pbar = tqdm(
        total=config.tot_steps,
        leave=True,
        desc="Current Loss: N/A; Current KNN Acc: N/A",
    )
        ...
        for batch_images, batch_dimensions in train_loader:
            ...
            if num_steps % 5000 == 0:
                ...
                torch.save(
                    ...
                    f"{save_dir}/univit{save_name}.pt",
                )
    ...
    pickle.dump(knn_plot, open(f"{save_dir}/univit{save_name}_knn_plot.pkl", "wb"))
