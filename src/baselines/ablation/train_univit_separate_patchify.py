# Show one of our contributions is a shared patchify stem allowing shared knowledge across modalities 
# (2D is a special case of 3D is a special cast of 4D)
...
if os.path.exists(f"{save_dir}/univit_separate.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'{save_dir}/univit_separate.pt', map_location='cpu')
...
while num_steps < config.tot_steps:
    ...
    for batch_images, batch_dimensions in train_loader:
        ...
        if num_steps % 1000 == 0:
            ...
            torch.save({'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'steps': num_steps}, f'{save_dir}/univit_separate.pt')
        if num_steps >= config.tot_steps:
            break
  
pbar.close()
pickle.dump(loss_plot, open(f'{save_dir}/univit_separate_loss_plot.pkl', 'wb'))
