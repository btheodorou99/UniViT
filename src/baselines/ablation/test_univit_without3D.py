...
tune_data['PET'] = tune_data['Amyloid PET'] + tune_data['FDG PET']
...
test_data['PET'] = test_data['Amyloid PET'] + test_data['FDG PET']
task_map = pickle.load(open(f"{data_dir}/taskMap.pkl", "rb"))
task_map['PET'] = task_map['Amyloid PET']
...
for task in tasks_3D:
    ...
    task_test_data = ImageDataset(
        task_test, config, "cpu", augment=False, downstream=True, multiclass=multiclass, depth=False
    )
