...
from src.data.image_dataset_withoutDepth import ImageDataset
...
tasks_3D = ['DeepLesion', 'ADNI PET', 'ADNI MRI', 'ACDC']
...
for task in tasks_3D:
    ...
    task_data = ImageDataset(
        task_data, config, "cpu", augment=False, downstream=True, multiclass=multiclass
    )
    ...
    for fold in tqdm(range(config.downstream_folds), desc=f"Evaluating {task} Folds"):
...
pickle.dump(allResults, open(f"{save_dir}/{model_key}_withoutDepth_downstreamResults.pkl", "wb"))
