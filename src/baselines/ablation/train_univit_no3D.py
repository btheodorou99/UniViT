...
from src.data.image_dataset_no3D import ImageDataset, KNNDataset
...
def frameSim_loss_fn(time_seq1, time_seq2, time_mask):
...
while num_steps < config.tot_steps:
    ...
    for batch_images, batch_dimensions in train_loader:
        ...
        cls_model1, embd_seq_model1, mask1, orig_mask1, time_mask1 = model(
            batch_images1.to(device),
            batch_dimensions1.to(device),
            seqMask=True,
            train=True,
        )
        cls_model2, embd_seq_model2, mask2, orig_mask2, time_mask2 = model(
        ...
        embd_seq_model1 = embd_seq_model1[:, config.max_time :]
        time_seq_model2 = embd_seq_model2[:, : config.max_time]
        embd_seq_model2 = embd_seq_model2[:, config.max_time :]
        with torch.no_grad():
            ...
            embd_seq_teacher1 = embd_seq_teacher1[:, config.max_time :]
            ...
            embd_seq_teacher2 = embd_seq_teacher2[:, config.max_time :]
...
pickle.dump(knn_plot, open(f"{save_dir}/univit_no3D_knn_plot.pkl", "wb"))
