...
from src.models.univit_noTime import UniViT
from src.data.image_dataset_noTime import ImageDataset
...
if os.path.exists(f"{save_dir}/univit_noTime.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'{save_dir}/univit_noTime.pt', map_location='cpu')
...
while num_steps < config.tot_steps:
    ...
    for batch_images, batch_dimensions in train_loader:
        ...
        if num_steps % 1000 == 0:
            ...
            torch.save({'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'steps': num_steps}, f'{save_dir}/univit_noTime.pt')
        if num_steps >= config.tot_steps:
            break
  
pbar.close()
pickle.dump(loss_plot, open(f'{save_dir}/univit_noTime_loss_plot.pkl', 'wb'))
