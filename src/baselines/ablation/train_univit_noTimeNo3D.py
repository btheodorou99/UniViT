...
from src.data.image_dataset_noTimeNo3D import ImageDataset, KNNDataset
...
if os.path.exists(f"{save_dir}/univit_noTimeNo3D.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f"{save_dir}/univit_noTimeNo3D.pt", map_location="cpu")
...
while num_steps < config.tot_steps:
    ...
    for batch_images in train_loader:
        ...
        cls_model1, embd_seq_model1, mask1 = model(
            batch_images1.to(device), seqMask=True, train=True
        )
        cls_model2, embd_seq_model2, mask2 = model(
            batch_images2.to(device), seqMask=True, train=True
        )
        with torch.no_grad():
            cls_teacher1, embd_seq_teacher1, _ = teacher_model(
                batch_images1.to(device), seqMask=False, train=True
            )
            cls_teacher1 = cls_teacher1.to(device)
            embd_seq_teacher1 = embd_seq_teacher1.to(device)
            cls_teacher2, embd_seq_teacher2, _ = teacher_model(
                batch_images2.to(device), seqMask=False, train=True
            )
        ...
        loss_mim = (
            mim_loss_fn(embd_seq_model1, embd_seq_teacher1, center_patch, mask1)
            + mim_loss_fn(embd_seq_model2, embd_seq_teacher2, center_patch, mask2)
        ) / 2
        ...
        if num_steps % 5000 == 0:
            ...
            torch.save(
                ...
                f"{save_dir}/univit_noTimeNo3D.pt",
            )
        if num_steps >= config.tot_steps:
            break

pbar.close()
pickle.dump(loss_plot, open(f"{save_dir}/univit_noTimeNo3D_loss_plot.pkl", "wb"))
pickle.dump(knn_plot, open(f"{save_dir}/univit_noTimeNo3D_knn_plot.pkl", "wb"))
