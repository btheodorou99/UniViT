...
from src.models.direct import Embedding
...
train_data = [p for p in pickle.load(open(f'{data_dir}/trainingDataset.pkl', 'rb')) if len(p) > 1 and p[-1][4] is not None and p[0][3] == 'MRI']
tune_data = {task: [p for p in tune_data[task] if p[-1][4] is not None] + train_data for task in tune_data if task == 'MRI'}
...
for task in tune_data:
    ...
    task_test_data = ImageDataset(task_test, config, 'cpu', augment=False, downstream=True, multiclass=multiclass)
    ...
    model1 = Embedding(label_size).to(device)
    model2 = Embedding(label_size, temporal=True).to(device)
    optimizer1 = torch.optim.Adam(model1.parameters(), lr=config.downstream_lr)
    optimizer2 = torch.optim.Adam(model2.parameters(), lr=config.downstream_lr)
    for epoch in tqdm(range(config.downstream_epochs), leave=False, desc=f'{task} Tuning'):
        batches_since_step = 0
        for batch_image1, batch_image2, batch_labels in tqdm(task_tune_loader, desc=f'{task} Tuning Epoch {epoch+1}', leave=False):
    ...
    task_preds2 = []
    task_labels = []
    for batch_image1, batch_image2, batch_labels in tqdm(task_test_loader, desc=f'{task} Testing', leave=False):
        batch_image1 = batch_image1.to(device)
        batch_image2 = batch_image2.to(device)
        batch_labels = batch_labels.to(device)
        with torch.no_grad():
    ...
    if taskType == 'Multi-Label Classification':
        ...
        print("STATIC")
        accPerLabel = [metrics.accuracy_score([label[i] for label in task_labels], [pred[i] for pred in task_rounded_preds1]) for i in range(label_size)]
        f1PerLabel = [metrics.f1_score([label[i] for label in task_labels], [pred[i] for pred in task_rounded_preds1]) for i in range(label_size)]
        aurocPerLabel = [metrics.roc_auc_score([label[i] for label in task_labels], [pred[i] for pred in task_preds1]) for i in range(label_size)]
        overallAcc = metrics.accuracy_score(task_labels.flatten(), task_rounded_preds1.flatten())
        overallF1 = metrics.f1_score(task_labels.flatten(), task_rounded_preds1.flatten())
        overallAUROC = metrics.roc_auc_score(task_labels.flatten(), task_preds1.flatten())
        ...
        print(taskResults)
        print("TEMPORAL")
        accPerLabel = [metrics.accuracy_score([label[i] for label in task_labels], [pred[i] for pred in task_rounded_preds2]) for i in range(label_size)]
        f1PerLabel = [metrics.f1_score([label[i] for label in task_labels], [pred[i] for pred in task_rounded_preds2]) for i in range(label_size)]
        aurocPerLabel = [metrics.roc_auc_score([label[i] for label in task_labels], [pred[i] for pred in task_preds2]) for i in range(label_size)]
        overallAcc = metrics.accuracy_score(task_labels.flatten(), task_rounded_preds2.flatten())
        overallF1 = metrics.f1_score(task_labels.flatten(), task_rounded_preds2.flatten())
        overallAUROC = metrics.roc_auc_score(task_labels.flatten(), task_preds2.flatten())
        taskResults = {'Accuracy': overallAcc, 'F1': overallF1, 'AUROC': overallAUROC, 'Accuracy Per Label': accPerLabel, 'F1 Per Label': f1PerLabel, 'AUROC Per Label': aurocPerLabel}
        print(taskResults)
    elif taskType == 'Multi-Class Classification':
        task_preds1 = np.array(task_preds1)
