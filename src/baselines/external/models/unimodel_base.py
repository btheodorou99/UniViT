...
class DeepPrompt(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.target_prompt = (
            cfg.MODEL.PROMPT_EMBED.TARGET_DEEP_PROMPT
            and not cfg.MODEL.PROMPT_EMBED.SHARE_DEEP_PROMPT
        )
        self.embedding = torch.nn.Embedding(
            cfg.MODEL.PROMPT_EMBED.INPUT_DEEP_PROMPT_LENGTH, embedding_hidden_size
        )
        if self.target_prompt:
            self.target_embedding = torch.nn.Embedding(
                cfg.MODEL.PROMPT_EMBED.TARGET_DEEP_PROMPT_LENGTH, embedding_hidden_size
            )
...
class TransformerEncoderLayer(nn.Module):
    ...
    __constants__ = [
        "batch_first",
        "norm_first",
    ]  # we inherit this variable from pytorch's code for jit
    ...
    def __init__(
        ...
        if self.layer_scale:
            self.gamma_1 = nn.Parameter(
                ls_init_values * torch.ones((d_model)), requires_grad=True
            )
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None):
        ...
        if self.norm_first:
            ...
            x = x + self._sa_block(
                self.norm1(x), src_mask, None, history_states=history_states_norm
            )
    ...
    def _sa_block(
        ...
        **kwargs,
        ...
        if self.deep_prompt:
            ...
            deep_prompt_embedding = self.deep_prompt_embedding(
                x,
                batch_first=(self.batch_first and self._torch_nn_new_interface),
                **kwargs,
            )
            if self.norm_first:
                deep_prompt_embedding = self.norm1(deep_prompt_embedding)
            kv = torch.cat(
                [deep_prompt_embedding, kv],
                dim=1 if (self.batch_first and self._torch_nn_new_interface) else 0,
            )
            if attn_mask is not None:
                L, S = attn_mask.shape
                pe_length = deep_prompt_embedding.shape[
                    1 if (self.batch_first and self._torch_nn_new_interface) else 0
                ]  # length, bs, hidden_size
                attn_mask = torch.cat(
                    [
                        torch.zeros(
                            (L, pe_length),
...
class SinusoidEncoding(nn.Module):
    def __init__(self, dim, max_len):
        ...
        div_term = torch.exp(
            torch.arange(0, dim, 2).float() * -(math.log(max_len * 2.0) / dim)
        )
    ...
    def forward(self, x, start_time=0):
        if isinstance(x, int):
            position_embeddings = self.position_embeddings(
                torch.tensor([x], dtype=torch.long).cuda()
            )
            ...
            x_size = x.size(1)
            position_ids = (
                torch.arange(x_size, dtype=torch.long, device=x.device) + start_time
            )
...
class TokenBaseEmbedding(nn.Module):
    ...
    def __init__(self, dim=768, vocab_size=49411, **kwargs):  # include <BOS>/<EOS>
        super(TokenBaseEmbedding, self).__init__()
        kwargs = {"dim": 768, "vocab_size": 49411}
    ...
    def forward(self, input_ids):
        ...
        if self.s_token_bias is not None:
            # learnable
            embeddings[input_ids == 49410] = (
                embeddings[input_ids == 49410] + self.s_token_bias
            )
        ...
        if self.embeddings_token_type is not None:
            ...
            embeddings_token_type = (
                self.embeddings_token_type.weight[0].unsqueeze(0).unsqueeze(1)
            )
...
class VideoBaseEmbedding(nn.Module):
    ...
    def __init__(
        ...
        max_spatial_size = (
            (input_size_3D[0] // patch_size)
            * (input_size_3D[1] // patch_size)
            * (input_size_3D[2] // patch_size)
        )
        max_spatial_size_2D = (input_size_2D[0] // patch_size) * (
            input_size_2D[1] // patch_size
        )
        ...
        self.embeddings_st_pos_2D = Divide_ST_POS(
            max_spatial_size_2D, 8, out_dim, self.random_temporal_pos
        )
        self.embeddings_st_pos_3D = Divide_ST_POS(
            max_spatial_size, 8, out_dim, self.random_temporal_pos
        )
        # self.embeddings_pos = None
        del self.embeddings

        self.embeddings = nn.Conv2d(
            3, out_dim, kernel_size=self.patch_size, stride=self.patch_size
        )
        self.embeddings_3D = nn.Conv3d(
            1, out_dim, kernel_size=self.patch_size, stride=self.patch_size
        )
    ...
    def forward(self, data):
        ...
        if data.dim() == 4:
            ...
            embeddings = rearrange(
                x, "(b t s) c hw -> b t hw (s c)", b=bs, s=self.time_span
            )
            # print(embeddings.size())
            embeddings_pos = (
                self.embeddings_st_pos_2D(embeddings).unsqueeze(0).flatten(1, 2)
            )
            ...
            # print(x.size())
            embeddings = rearrange(
                x, "(b t s) c dhw -> b t dhw (s c)", b=bs, s=self.time_span
            )
            # print(embeddings.size())
            embeddings_pos = (
                self.embeddings_st_pos_3D(embeddings).unsqueeze(0).flatten(1, 2)
            )
            embeddings = embeddings.flatten(1, 2)
            if self.pos_before:
                embeddings = embeddings + embeddings_pos.to(embeddings.dtype)
    ...
    def forward(self, x):
        ...
        if self.training and self.random_temporal_pos:
            temporal_pos_ids = torch.arange(
                temp_len, dtype=torch.long, device=x.device
            ) + torch.randint(
                0,
                self.max_frames - temp_len + 1,
                size=(1,),
                dtype=torch.long,
                device=x.device,
            )
        ...
        pos_embed = self.temporal_pos_embed(temporal_pos_ids).unsqueeze(1).to(
            dtype=dtype
        ) + self.spatial_pos_embed(
            torch.arange(
                start=self.spatial_pos_embed_index,
                end=spatial_size + self.spatial_pos_embed_index,
                dtype=torch.long,
                device=x.device,
            )
        ).unsqueeze(
