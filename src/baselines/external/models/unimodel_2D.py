...
from src.baselines.external.models.unimodel_base import (
...
class Unified_Model(nn.Module):
    def __init__(
        ...
        if pre_trained:
            ...
            print(
                "[pre_%d/mod_%d]: %d shared layers"
                % (len(pre_dict), len(model_dict), len(pre_dict_update))
            )
    ...
    def forward(self, data):
        ...
        #                 data["labels"].cpu().numpy(), cls_logits_argmax.cpu().numpy()
        #             )
