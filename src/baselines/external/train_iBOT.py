...
import json
import numpy as np
import src.baselines.external.ibot.utils as utils
import src.baselines.external.ibot.models as models
import torch
...
from src.baselines.external.ibot.models.head import iBOTHead
from src.baselines.external.ibot.loader import ImageFolderMask
from src.baselines.external.ibot.evaluation.unsupervised.unsup_cls import eval_pred
...
def train_ibot(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        utils.save_on_master(save_dict, os.path.join(args.output_dir, 'ibot.pt'))
        if args.saveckp_freq and (epoch % args.saveckp_freq == 0) and epoch:
            utils.save_on_master(save_dict, os.path.join(args.output_dir, f'ibot.pt'))
...
if __name__ == '__main__':
    ...
    args.out_dim = config.projection_size
    args.patch_out_dim = config.projection_size
    args.shared_head = True
    args.shared_head_teacher = True
    args.norm_last_layer = True
    args.momentum_teacher = config.momentum
    args.pred_ratio = [0.0, 0.3]
    args.pred_ratio_var = [0.0, 0.2]
    args.pred_shape = 'block'
    args.pred_start_epoch = 0
    args.lambda1 = 1.0
    args.lambda2 = 1.0
    args.warmup_teacher_temp = config.teacher_cls_temp
    args.teacher_temp = config.teacher_cls_temp
    args.warmup_teacher_patch_temp = config.teacher_patch_temp
    args.teacher_patch_temp = config.teacher_patch_temp
    args.warmup_teacher_temp_epochs = 0
    args.use_fp16 = False
    args.weight_decay = 0.01
    args.weight_decay_end = 0.01
    args.clip_grad = 3.0
    args.batch_size_per_gpu = config.batch_size
    args.epochs = config.tot_epochs
    args.freeze_last_layer = 3
    args.lr = config.lr
    args.warmup_epochs = 0
    args.min_lr = config.lr
    args.optimizer = 'adamw'
    args.data_path = data_path
    args.output_dir = save_dir
    args.load_from = 'ibot.pt'
    args.saveckp_freq = 1
    args.seed = SEED
