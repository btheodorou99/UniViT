...
from src.baselines.external.data.buffer_dataset import (
    ImageDataset,
    BufferDataset,
    KNNDataset,
)
...
for modality in modalities:
    ...
    train_data = BufferDataset(
        [(p, 0) for p in train_data_modality] + [(p, 1) for p in buffer], config, "cpu"
    )
    ...
    while num_steps < config.tot_steps:
        ...
        for batch_images, batch_dimensions, batch_buffer_mask in train_loader:
            batch_images, batch_dimensions = train_data.augment_batch(
