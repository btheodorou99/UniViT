...
class ImageDataset(Dataset):
    ...
    def adjust_size(self, origDim):
        ...
        newHeight = min(
            max(int(origDim[0] * scale_factor_image), MIN_RESOLUTION),
    ...
    def load_image(self, image_path, chosenDim):
        ...
        if chosenDim is not None and chosenDim != currDim:
            ...
            elif (currDim[0] > chosenDim[0]) == (chosenDim[1] > currDim[1]) and (
                currDim[0] != chosenDim[0] and currDim[1] != chosenDim[1]
            ):
    ...
    def random_crop(self, img, dim):
        crop_height = random.randint(
            int(dim[1] * MIN_CROP), img.shape[2]
    ...
    def random_resize(self, img, dim):
        ...
        img[:, :, :height, :width] = F.interpolate(
            img[:, :, : dim[1], : dim[2]].view(
                -1, self.config.num_channels, dim[1], dim[2]
            ),
            size=(height, width),
            mode="bilinear",
            align_corners=False,
        ).view(img.shape[0], self.config.num_channels, height, width)
        img[:, :, height:, :] = 0
        img[:, :, :, width:] = 0
        dim[1] = height
        dim[2] = width
