...
class SegmentationModel(nn.Module):
    def __init__(self, representation_dim, patch_size, slice=1, fullSlice=False):
        super(SegmentationModel, self).__init__()
        self.representation_dim = representation_dim
        self.hidden_dim = 64
        self.patch_size = patch_size
        self.slice = slice
        self.fullSlice = fullSlice
        ...
        if self.slice > 1:
            self.deconv1 = nn.ConvTranspose3d(self.hidden_dim, self.hidden_dim, kernel_size=(1 if self.fullSlice else self.slice, self.deconv_schedule[0], self.deconv_schedule[0]), stride=(1 if self.fullSlice else self.slice, self.deconv_schedule[0], self.deconv_schedule[0]), padding=0)
    ...
    def forward(self, x):
        if self.slice > 1 and self.fullSlice:
