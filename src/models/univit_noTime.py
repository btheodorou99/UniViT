...
class UniViT(nn.Module):
    def __init__(
        ...
        if posTypeHead:
            ...
            self.pos_head = nn.Sequential(nn.Linear(representation_size, representation_size), nn.ReLU(), nn.Linear(representation_size, self.image_height + self.image_width + self.image_slice))
    ...
    def _process_input(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # (b, s, c, h, w) -> (b, c, s, h, w) -> (b, hidden, n_h * n_w)
