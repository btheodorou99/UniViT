...
class UniViT(nn.Module):
    def __init__(
        ...
        if posTypeHead:
            ...
            self.pos_head = nn.Sequential(nn.Linear(representation_size, representation_size), nn.ReLU(), nn.Linear(representation_size, self.image_height + self.image_width))
    ...
    def _prepare_sequence(self, x: torch.Tensor, dimensions: torch.Tensor, swapPos: bool = False, adversarialPos = None, adversarialPatch = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(bs):
            mask[i, math.ceil(dimensions[i,0] / self.patch_size):, :] = True
