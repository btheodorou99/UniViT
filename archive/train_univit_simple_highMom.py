...
if os.path.exists(f"{save_dir}/univit_simple_highMom.pt"):
    print("Loading previous model")
    checkpoint = torch.load(f'{save_dir}/univit_simple_highMom.pt', map_location='cpu')
...
while num_steps < config.tot_steps:
    ...
    for batch_images, batch_dimensions in train_loader:
        ...
        cls_model = model.embed(batch_images_aug1.to(device), batch_dimensions1.to(device), train=True)
        embd_seq_model, mask, orig_mask = model(batch_images.to(device), batch_dimensions.to(device), seqMask=True, train=True)
        with torch.no_grad():
        ...
        if num_steps % 1000 == 0 and batches_since_step % batches_per_step == 0:
            ...
            torch.save({'model': model.state_dict(), 'optimizer': optimizer.state_dict(), 'steps': num_steps}, f'{save_dir}/univit_simple_highMom.pt')
        if num_steps >= config.tot_steps:
            break
  
pbar.close()
pickle.dump(loss_plot, open(f'{save_dir}/univit_simple_highMom_loss_plot.pkl', 'wb'))
